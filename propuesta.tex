...
\begin{abstract}
    ...
    de trios \textit{sujeto - predicado - objeto} a través del formato \rdf. Sin embargo,
    para un usuario con pocos conocimientos sobre la web semántica, escribir una consulta
    \spql es una tarea que implica un alto esfuezo cognitivo. Se han desarrollado herramientas
    y plataformas para apoyar a los usuarios en la tarea de explorar las bases de datos y
    construir consultas \spql, tales como el proyecto \textit{RDF Explorer}. Pero aún no es
    suficiente, puesto que estas herramientas continuan exigiendo al usuario algún grado de
    conocimiento sobre el esquema y los contenidos de la base de datos consultada. Una de
    las soluciones a esta problematica es proveer al usuario con recomendaciones sobre las
    entidades que existen y que puede consutar en la base de datos. De las multiples
    soluciones existentes, nos enfocaremos en el proyecto \textit{SPARQLforHumans}. En este
    trabajo buscamos diseñar e implementar mejoras al rendimiento del proyecto
    \textit{SPARQLforHumans} con una restricción importante, solo podemos utilizar software
    de código abierto para nuestra implementación.
\end{abstract}
